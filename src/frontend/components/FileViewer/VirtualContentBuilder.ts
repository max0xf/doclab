...
export class VirtualContentBuilder {
  ...
  private processDiffHunk(
    ...
    let lastDiffType: 'addition' | 'deletion' | null = null;
    ...
    hunk.lines.forEach((hunkLine: string) => {
      ...
      if (prefix === '-') {
        // Deletion
        const isFirst = lastDiffType !== 'deletion';
        lines.push({
        ...
        lastDiffType = 'deletion';
      } else if (prefix === '+') {
        // Addition
        const isFirst = lastDiffType !== 'addition';
        lines.push({
          lineNumber: originalLineNumber,
          virtualLineNumber: 0, // Will be set later
          content,
  ...
  private isDiffEnrichment(enrichment: Enrichment): boolean {
    return ['pr_diff', 'commit', 'edit_session', 'diff'].includes(enrichment.type);
  }
