...
import { FileText, Folder, ChevronRight, ChevronDown, Loader2 } from 'lucide-react';
...
function TreeNodeItem({
  ...
  const handleClick = async () => {
    if (node.type === 'dir') {
      ...
      console.log('DocumentTree: Navigating to folder:', node.path);
      ...
      console.log('DocumentTree: Navigating to file:', node.path, 'title:', node.title);
      navigate(`${Urls.Repositories}?repo=${repositoryId}&path=${encodeURIComponent(node.path)}`);
    }
  };
  ...
  return (
    <div>
      <button
        onClick={handleClick}
        className="w-full flex items-center gap-1.5 py-1.5 px-2 text-xs hover:bg-opacity-10 hover:bg-gray-500 rounded transition-all"
        style={{
      ...
      >
        ...
        {node.type === 'file' && <FileText className="w-3.5 h-3.5 flex-shrink-0 ml-3" />}
...
export default function DocumentTree({ repositoryId, currentPath }: DocumentTreeProps) {
  ...
  useEffect(() => {
    const fetchTree = async () => {
      try {
        ...
        const response = await wikiApi.getRepositoryTree(repositoryId, 'document', defaultBranch);
  ...
  if (tree.length === 0) {
    return (
      <div className="mt-2 px-4 py-2 text-xs" style={{ color: 'var(--text-muted)' }}>
        No documents found
      </div>
    );
  }
